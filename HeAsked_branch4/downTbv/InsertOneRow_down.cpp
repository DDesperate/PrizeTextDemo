...
//混合模式下，装奖注的数据结构中顺序并不直接按照prize大小顺序排列，需要用下述两个函数调整
bool compareByPrize(const slcInfo &info1, const slcInfo &info2) {
    return info1.prize < info2.prize;
}

void sortSelectMixDataTableRow(QList<slcInfo> &dataList) {
    std::sort(dataList.begin(), dataList.end(), compareByPrize);
}
...
void DownTableView::insertRow(const QString &date,const QList<quint8> &prizes)
{
    ...
    sortSelectMixDataTableRow(selectMixDataTableRow);
