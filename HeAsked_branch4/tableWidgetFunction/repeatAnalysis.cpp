...
bool sortBySizeAndStr(const QPair<QString,QVector<QString>> &v1, const QPair<QString,QVector<QString>> &v2)
{
    //期数越多越靠前，期数相等时按照奖注由小到大排序
    return (v1.first.size() > v2.first.size()) || (v1.first.size() == v2.first.size() && v1.second < v2.second);
}

void Widget::repeatAnalysis(QTableWidget *tbwgt, QLineEdit *le, QTextBrowser *tb)
{
    tb->clear();
    ...
    for(int row=0; row < (analysisNumSpinBox->value()>tbwgt->rowCount()?tbwgt->rowCount():analysisNumSpinBox->value()); row++)
    {
        ...
        for(int col=1; col < tbwgt->columnCount(); col++)
        {
            if(prize.contains(tbwgt->item(row,col)->text())){
                tbwgt->item(row,col)->setBackground(QColor(0,255,0));//高亮可以不要
    ...
    //上述代码相当于PrizeSearch(tbwgt,le);的功能，不用PrizeSearch(tbwgt,le);代替是因为需要获取到Res
    //Res表示所有的标绿项
